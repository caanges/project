...
class TurtleBot(Node):
    def __init__(self):
        ...
        """
        #self.waypoints = self.get_parameter("waypoints").get_parameter_value().byte_array_value
        ...
        self.current_waypoint_index = 0.0
        '''
...
def main():
    ...
    with open('path_coordinates.json', 'r') as file:
            data = json.load(file)
    x = []
    y = []
    i = 0
    ...
    try:
        ...
        for i in range(len(x)):
            ...
            turtlebot.get_logger().info(f"moving to: x = {x[i]}, y = {y[j]}")
            turtlebot.goal_pose.x = x[i]
            turtlebot.goal_pose.y = y[j]
            turtlebot.moving_to_goal = True

            while turtlebot.moving_to_goal:
                time.sleep(0.5)

            #time.sleep(1.0) # pause at each waypoint


        turtlebot.get_logger().info("All waypoints done")
